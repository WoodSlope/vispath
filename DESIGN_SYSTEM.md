...
| `>= 1000px` | 单条结果使用图片/信息横向宽版布局 |
| `>= 1180px` | 三/五/六套方案使用三列；结果工具栏横向排列 |
| `901-1280px` | 结果批次两列，面板保持 `16px` 内边距 |
...
| 批次 | `.generation-batch`, `.generation-batch-summary` | 批次头 `52px`；统计、对比、删除属于批次 | collapsed, expanded, loading, deletable, disabled |
| 结果卡 | `.generation-card`, `.generation-art`, `.generation-body` | 标题/媒体/摘要/动作固定顺序；媒体比例稳定 | loading, ready, error, favorite, long-content |
